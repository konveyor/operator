...
bundle: helm operator-sdk ## Generate bundle manifests and metadata, then validate generated files.
	$(HELM) template --set images.operator=${IMG} --set version=$(VERSION) --set olm=true $(HELM_OPTS) ./helm | $(OPERATOR_SDK) generate bundle --extra-service-accounts tackle-hub,tackle-ui,agentic-controller,agent-sandbox-controller $(BUNDLE_GEN_FLAGS)
